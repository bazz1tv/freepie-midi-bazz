def update():
    ...
    if midi[2].data.channel == 0:
        if midi[2].data.status == MidiStatus.NoteOn:
            ...
            key = 0
            # Down
            if midi[2].data.buffer[0] == (22+12+12+12+12) or midi[2].data.buffer[0] == (21+12+12+12+12): # A-1
                key = Key.DownArrow
            # Up
            if midi[2].data.buffer[0] == (24+12+12+12+12) or midi[2].data.buffer[0] == (23+12+12+12+12): 
                key = Key.UpArrow # A#1
            # Left
            if midi[2].data.buffer[0] == (26+12+12+12+12):
                key = Key.LeftArrow # D-1
            # Right
            if midi[2].data.buffer[0] == (28+12+12+12+12):
            ...
            if midi[2].data.buffer[0] == (26+12+12+12+12+12):
                key = Key.A # D-2
            # X
            if midi[2].data.buffer[0] == (35+12+12+12+12) or midi[2].data.buffer[0] == (36+12+12+12+12)or midi[2].data.buffer[0] == (37+12+12+12+12):
                key = Key.S # G-2
            # L
            if midi[2].data.buffer[0] == (29+12+12+12+12+12):
                key = Key.Q # G-2
            # R
            if midi[2].data.buffer[0] == (30+12+12+12+12+12):
                key = Key.W # B-2
            # Select
            if midi[2].data.buffer[0] == (19+12+12+12):
                key = Key.E # A-0
            # Start
            if midi[2].data.buffer[0] == (31+12+12):
                key = Key.R # B-0
